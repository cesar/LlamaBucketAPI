...
exports.get_filtered_results = function(req, res, next) {
	...
	var query = 'select *, count(B.listing_id) as bid_count from bidding_history as B RIGHT JOIN (select * from listing natural join item natural join category where item_category = cat_id and item_name like and NOW() < exp_date ' + connection.escape(search_param) + type_query + price_filter_query + ') as T ON B.listing_id = T.listing_id  and listing_is_active = 1 group by T.listing_id ' + sort_by_query;
...
exports.get_filtered_category_results = function(req, res, next) {
	...
	var query = 'select *, count(B.listing_id) as bid_count from bidding_history as B RIGHT JOIN (select * from listing natural join item natural join category where item_category =' + connection.escape(req.body.cat_id) + type_query + price_filter_query + ' and NOW() < exp_date) as T ON B.listing_id = T.listing_id and listing_is_active = 1  group by T.listing_id ' + sort_by_query;
...
exports.get_results = function(req, res, next) {
	...
	if (query_parameter.search(re_category) >= 0) {
		...
		connection.query('select *, count(B.listing_id) as bid_count from bidding_history as B RIGHT JOIN (select * from listing natural join item natural join category where item_category=' + connection.escape(cat) + ' and listing_is_active = 1 and NOW() < exp_date and item_category = cat_id) as T ON B.listing_id = T.listing_id  group by T.listing_id', function(err, rows) {
			res.send({
				content: rows
			});
			console.log({
		...
		connection.query('select *, count(B.listing_id) as bid_count from bidding_history as B RIGHT JOIN (select * from listing natural join item where item_name like ' + connection.escape(search_terms) + ' and listing_is_active = 1 and NOW() < exp_date) as T ON B.listing_id = T.listing_id   group by T.listing_id', function(err, rows) {
			if (!err) {
				res.send({
					content: rows
				});
