...
exports.insert_notification = function(req, res, next){
	...
	new_lid = client_listing_id_string.substring(stop, client_listing_id_string.length);
	...
	connection.beginTransaction(function(err) {
		...
		connection.query('select * from listing natural join item where listing_id = ' + new_lid, function(err, rows){
			if(!err){
				...
				var insert_notification_query = 'insert into user_notifications (client_id, listing_id, is_read, notification_message, notification_date, title)values ('+ connection.escape(new_cid) + ', '+ connection.escape(new_lid) + ', 0, "' + connection.escape(new_message) + '", now(), "Sold")';
