        ...
        connection.query(final_query, function(err, third_result) {
          ...
          connection.query(get_client, function(err, fourth_result) {
            ...
            for (var k = 0; k < first_result.length; k++) {
              ...
              values2 = values2 + '(now(), ' + first_result[k].price + ', ' + connection.escape(req.params.id) + ', ' + first_result[k].listing_id + ', ' + fourth_result[0].cc_id + ', ' + fourth_result[0].address_id + '), ';
            }
    ...
    connection.query(update_listing, function(err, first_result) {
      ...
      connection.query(get_user_information, function(err, second_result) {
        ...
        connection.query(get_listing_information, function(err, third_result) {
          ...
          var invoice = 'insert into invoice (invoice_date, final_price, buyer_id, listing_id, credit_card, shipping_address) values (now(), ' + third_result[0].price + ', ' + connection.escape(req.body.id) + ', ' + third_result[0].listing_id + ', ' + second_result[0].cc_id + ', ' + second_result[0].address_id + ')';

          var notification = 'insert into user_notifications (client_id, listing_id, is_read, notification_message, notification_date, title) values (' + third_result[0].seller_id + ', ' + third_result[0].listing_id + ', 0, "Item sold", now(), "Sold")';
