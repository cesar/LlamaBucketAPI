 ...
 */
exports.get_cart = function(req, res, err) {
	var query = 'select * from item natural join (select item_id, price, listing_id from listing where listing_is_active = 1 and is_auction = "buy" or is_auction = "both") as t1 natural join (select listing_id, client_id from bucket where client_id = ' + connection.escape(req.params.id) + ') as t2';
 ...
 *
 */
exports.bucket_checkout = function(req, res, next) {
	var query = 'select * from item natural join (select item_id, listing_id, price from listing where listing_is_active = 1) as t1 natural join (select listing_id, client_id from bucket where client_id = ' + connection.escape(req.params.id) + ') as t2 natural join (select client_id, address_1, address_2, city, state, country, zip_code from address where is_primary = 1) as t3 natural join (select client_id, cc_number, cc_type, billing_address from credit_card where is_primary = 1) as t4;'
 ...
 */
exports.item_checkout = function(req, res, next) {
	...
	var query_1 = 'select address_1, address_2, city, zip_code, state, country, cc_number, cc_type from address natural join credit_card where client_id = ' + client_id + ' and is_primary = 1';
...
exports.place_order_bucket = function(req, res, next) {
	var today = new Date();

	var query_1 = 'select * from item natural join (select item_id, listing_id, seller_id, price from listing where listing_is_active = 1) as t1 natural join (select listing_id from bucket where client_id = ' + connection.escape(req.params.parameter) + ') as t2 natural join (select client_firstname, client_lastname, client_id as seller_id from client) as t3;'

	var query_2 = 'select * from address natural join (select client_id, cc_number, cc_type from credit_card) as t1 where client_id = ' + connection.escape(req.params.parameter) + ' and is_primary = 1; '
...
exports.place_order_item = function(req, res, next) {
	var parameters = req.params.parameter.split('-');

	//Parameter 0 is the item id
	var query_1 = 'select * from item natural join (select seller_id as client_id, item_id, price, src_address from listing) as t1 natural join (select client_firstname, client_lastname, client_id from client) as t2 where item_id = ' + connection.escape(parameters[0]);

	//Patameter 1 is the client id
	var query_2 = 'select address_1, address_2, city, zip_code, state, country, cc_number, cc_type from address natural join credit_card where client_id = ' + connection.escape(parameters[1]) + ' and is_primary = 1';
