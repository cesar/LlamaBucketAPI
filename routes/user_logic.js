...
exports.get_creditcard = function(req, res, next) {
  var query = "select * from credit_card natural join (select address_1, address_2, city, state, zip_code, country, address_id as billing_address from address) as t1 where cc_id = " + connection.escape(req.params.id);
  connection.query(query, function(err, rows) {
    send_data = {
      id: rows[0].cc_id,
...
exports.get_creditcard_list = function(req, res, next) {
  var query = 'select cc_id, cc_number, cc_type, cc_exp_date, cc_holder, address_1, address_2, city, state, country, zip_code, is_primary from credit_card natural join (select address_id as billing_address, address_1, address_2, city, zip_code, country, state from address) as t1 where credit_card.is_active = 1 and client_id = ' + connection.escape(req.params.id) + ';'
...
exports.add_address = function(req, res, next) {
  var query = 'insert into address (address_1, address_2, city, zip_code, state, country, client_id, is_cc_active, is_active, is_primary) values (' + connection.escape(req.body.address1) + ', ' + connection.escape(req.body.address2) + ', ' + connection.escape(req.body.city) + ', ' + connection.escape(req.body.zipcode) + ', ' + connection.escape(req.body.state) + ', ' + connection.escape(req.body.country) + ', ' + connection.escape(parseInt(req.params.id)) + ', 0, 1, 0)';
...
exports.add_creditcard = function(req, res, next) {
  var address_query = 'insert into address (address_1, address_2, city, zip_code, state, country, client_id, is_primary, is_cc_active, is_active) values (' + connection.escape(req.body.billing_address1) + ', ' + connection.escape(req.body.billing_address2) + ', ' + connection.escape(req.body.billing_city) + ', ' + connection.escape(req.body.billing_zipcode) + ', ' + connection.escape(req.body.billing_state) + ', ' + connection.escape(req.body.billing_country) + ', ' + connection.escape(parseInt(req.params.id)) + ', 0, 1, 0)';
  ...
  connection.beginTransaction(function(err) {
    ...
    connection.query(address_query, function(err, result) {
      ...
      var creditcard_query = 'insert into credit_card (client_id, billing_address, cc_number, cc_type, cc_holder, cc_exp_date, is_primary, is_active) values (' + connection.escape(req.params.id) + ', ' + connection.escape(result.insertId) + ', ' + connection.escape(req.body.billing_cc_number) + ', ' + connection.escape(req.body.billing_cc_type) + ', ' + connection.escape(req.body.billing_cc_card_holder) + ', ' + connection.escape(req.body.billing_cc_exp_date) + ', 0, 1)';
...
exports.upload_item = function(req, res, err) {
  ...
  var item_insert_query = 'INSERT INTO item (item_name, item_image, item_description, item_brand, item_year, item_category) VALUES(' +
    connection.escape(req.body.item_name) + ',' + connection.escape(serverURL + '/' + req.files.file.ws.path) + ',' + connection.escape(req.body.item_description) + ',' + connection.escape(req.body.item_brand) + ',' + connection.escape(req.body.item_year) + ',' + connection.escape(req.body.item_category) + ')';
...
exports.register_user = function(req, res, next) {
  ...
  var new_user = 'insert into client (client_firstname, client_lastname, email, password, phone, client_image, user_active, isAdmin) values (' + connection.escape(req.body.register_firstname) + ', ' + connection.escape(req.body.register_lastname) + ', ' + connection.escape(req.body.register_email) + ', ' + connection.escape(req.body.register_password) + ', ' + connection.escape(req.body.register_phone) + ',' +
    connection.escape(serverURL + '/' + req.files.file.ws.path) + ', 1, 0)'
  ...
  connection.beginTransaction(function(err) {
    ...
    connection.query(new_user, function(err, first_result) {
      ...
      if (req.body.register_same_address === 'on') {
        var new_address = 'insert into address (address_1, address_2, city, zip_code, state, country, client_id, is_primary, is_cc_active, is_active) values (' + connection.escape(req.body.register_address1) + ', ' + connection.escape(req.body.register_address2) + ', ' + connection.escape(req.body.register_city) + ', ' + connection.escape(req.body.register_zipcode) + ', ' + connection.escape(req.body.register_state) + ', ' + connection.escape(req.body.register_country) + ', ' + connection.escape(first_result.insertId) + ', 1, 1, 1)';
      } else {
        var new_address = 'insert into address (address_1, address_2, city, zip_code, state, country, client_id, is_primary, is_cc_active, is_active) values (' + connection.escape(req.body.register_address1) + ', ' + connection.escape(req.body.register_address2) + ', ' + connection.escape(req.body.register_city) + ', ' + connection.escape(req.body.register_zipcode) + ', ' + connection.escape(req.body.register_state) + ', ' + connection.escape(req.body.register_country) + ', ' + connection.escape(first_result.insertId) + ', 1, 0, 1)';
      };
      ...
      connection.query(new_address, function(err, second_result) {
        ...
        var new_creditcard = 'insert into credit_card (client_id, cc_number, cc_type, cc_holder, cc_exp_date, billing_address, is_primary, is_active) values (' + connection.escape(first_result.insertId) + ', ' + connection.escape(req.body.register_creditcard_number) + ', ' + connection.escape(req.body.register_creditcard_type) + ', ' + connection.escape(req.body.register_firstname + ' ' + req.body.register_lastname) + ', ' + connection.escape(req.body.register_creditcard_expdate) + ', ' + connection.escape(second_result.insertId) + ', 1, 1)';
        ...
        connection.query(new_creditcard, function(err, thrird_result) {
          ...
          if (!req.body.register_same_address) {
            ...
            //Different addresses
            var new_billing_address = 'insert into address (address_1, address_2, city, zip_code, state, country, client_id, is_primary, is_cc_active, is_active) values (' + connection.escape(req.body.register_billing_address1) + ', ' + connection.escape(req.body.register_billing_address2) + ', ' + connection.escape(req.body.register_billing_city) + ', ' + connection.escape(req.body.register_billing_zipcode) + ', ' + connection.escape(req.body.register_billing_state) + ', ' + connection.escape(req.body.register_billing_country) + ', ' + connection.escape(first_result.insertId) + ', 0, 1, 0)';
            ...
            connection.query(new_billing_address, function(err, fourth_result) {
              ...
              var create_bank_account = 'INSERT INTO bank_account (account_owner, account_total, total_sales) VALUES(' + connection.escape(first_result.insertId) + ',0,0);';

              connection.query(create_bank_account, function(err, bank_callback) {
